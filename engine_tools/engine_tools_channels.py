...
class Heattransfer():
	...
	#TODO check adiabatic wall temperature 
	...
	def pressure_drop(self, surface_roughness, hydrolic_diameter, flow_velocity, section_lenght, y_coordinate):
		Re = self.coolant.rho*flow_velocity*hydrolic_diameter/self.coolant.mu
		fd = fsolve(lambda f: 1/(np.sqrt(f)) + 2*np.log10(surface_roughness/(3.7*hydrolic_diameter) + 2.51/(Re*np.sqrt(f))), 0.0000001)
		dp = fd*section_lenght/hydrolic_diameter*0.5*self.coolant.rho*flow_velocity**2 
	...
	def heat_trans_coeff_coolant(self, wall_temperature, x_coordinate, y_coordinate, hydrolic_diameter, flow_velocity, t_r, channel_height, channel_width):
		Pr = self.coolant.Pr
		Re = self.coolant.rho*flow_velocity*hydrolic_diameter/self.coolant.mu
		...
		fin_effectiveness = np.tanh(np.sqrt(2*halpha*t_r/k) * channel_height/t_r) / (np.sqrt(2*halpha*t_r/k) * channel_height/t_r)
		halpha_F = halpha * (channel_width+2*fin_effectiveness*channel_height)/(channel_width+t_r)
		...
		return halpha_F, Re, Nu

	def iterator(self, y_coordinate, x_coordinate, channel_width, channel_height, t_r, n_channels, section_lenght, initial_guess, mach, adiabatic_wall_temperature ,max_iter=1000, tol=1e-6):
		wall_temperature = 300
		iteration = 0
		difference = 1
		hydrolic_diameter = 2*channel_width*channel_height / (channel_width+channel_height)
		flow_velocity = self.coolant_massflow/(self.coolant.rho * np.pi * hydrolic_diameter**2/4 * n_channels)

		while difference > tol:
			halpha = self.heat_trans_coeff_gas(mach, wall_temperature, y_coordinate)
			halpha_c, Re, Nu = self.heat_trans_coeff_coolant(wall_temperature, x_coordinate, y_coordinate, hydrolic_diameter, flow_velocity, t_r, channel_height, channel_width)
		...
		dp = self.pressure_drop(0, hydrolic_diameter, flow_velocity, section_lenght, y_coordinate)
		self.coolant.calculate(P=self.coolant.P-dp, T=T_new)

		return heat_flux, wall_temperature, Re, Nu, radiation

	def heatflux(self, channel_width, channel_height, n_channels, geometry, max_temperature=0, optimise=False):
		"""determines heat flux along the entire geometry starting from the nozzle end. Calls iterator function for all grid points. Only use for engine without cooling channels. Can optimise cooling flow hydrolic diameter for a maximum wall temperature 
		...
		for i in range(len(y)):
			...
			t_r = (np.pi*2*(y[i]+self.wall_thickness) - channel_width*n_channels)/n_channels
			#t_r = 1e-3
			#channel_width = np.pi*2*(y[i]+self.wall_thickness)/n_channels - t_r
			...
			q, wall_temp, Re, Nu, radiation = self.iterator(y[i], x[i], channel_width, channel_height, t_r, n_channels, section_length, initial_guess[i], mach, adiabatic_wall_temperature)
