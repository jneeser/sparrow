...
class Heattransfer():
	...
	def __init__(self, coolant, coolant_massfraction, coolant_massflow, total_massflow, cea, chamber_pressure, coolant_temperature, coolant_pressure, geometry, number_of_channels, thermal_conductivity):
		"""[summary]
	...
	def iterator(self, y_coordinate, hydrolic_diameter, section_lenght, wall_thickness, mach, adiabatic_wall_temperature ,max_iter=1000, tol=1e-6):
	...
	def heatflux(self, hydrolic_diameter, geometry, wall_thickness, mach_numbers, adiabatic_wall_temperatures):
		...
		for i in range(len(y)):
			...
			q, wall_temp, Re, Nu, radiation, halpha = self.iterator(y[i], hydrolic_diameter[i], section_length, wall_thickness[i], mach_numbers[i], adiabatic_wall_temperatures[i])
